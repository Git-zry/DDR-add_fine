...
from utils.utils import icosphere_upsampling, bilinear_sphere_resample, resampleSphereSurf
...
class STN(nn.Module):
    def __init__(self, data_ico, labels_ico, control_ico, num_labels, device):
        ...
        if control_ico >= 4 or labels_ico >= 7:
            indices_matrix = torch.load(
                'neigh_ver/indices_red_' + str(control_ico) + '_' + str(labels_ico) + '.pt').type(torch.long)
        else:
        ...
        self.icosphere_labels_temp = torch.Tensor(nb.load(ico_dir + 'ico-' +
                                                          str(labels_ico) + '.surf.gii').darrays[0].data).to(
            self.device)
        ...
        self.neighs = torch.LongTensor(np.load(ddr_files_dir + 'hexagons_4.npy'))
        self.model_CRF = CrfRnn(self.num_labels, self.control_ver, self.device)

    def forward(self, moving_img, def_idxs,coarse_warp):
        ...
        def_cont_ico = torch.from_numpy(resampleSphereSurf(self.icosphere_control / 100.0, coarse_warp,
                                          def_cont_ico, None, False, None, self.neighs))#resample coarse deformed ico to fine deformed ico
